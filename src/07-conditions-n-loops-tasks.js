...
function isInsideCircle(circle, point) {
  const radius = (point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2;
  return radius < circle.radius ** 2;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${openBracket}${min}, ${max}${closeBracket}`;
}
...
function getDigitalRoot(num) {
  ...
  const sum = (item) => item.split('').reduce((acc, curr) => +acc + +curr, 0);
  while (str >= 10) {
    str = sum(str.toString());
  }
  return str;
}
